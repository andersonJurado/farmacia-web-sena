<template>
  <div>
    <h2 id="page-heading" data-cy="MunicipioHeading">
      <span v-text="$t('farmaciaApp.municipio.home.title')" id="municipio-heading">Municipios</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('farmaciaApp.municipio.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'MunicipioCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('farmaciaApp.municipio.home.createLabel')"> Create a new Municipio </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && municipios && municipios.length === 0">
      <span v-text="$t('farmaciaApp.municipio.home.notFound')">No municipios found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="municipios">
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('nombre')">
              <span v-text="$t('farmaciaApp.municipio.nombre')">Nombre</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'nombre'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('departamento.nombre')">
              <span v-text="$t('farmaciaApp.municipio.departamento')">Departamento</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'departamento.nombre'"></jhi-sort-indicator>
            </th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="municipio in municipios" :key="municipio.id" data-cy="entityTable">
            ...
            <td>
              <div v-if="municipio.departamento">
                <router-link :to="{ name: 'DepartamentoView', params: { departamentoId: municipio.departamento.id } }">{{
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="farmaciaApp.municipio.delete.question" data-cy="municipioDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-municipio-heading" v-text="$t('farmaciaApp.municipio.delete.question', { id: removeId })">
